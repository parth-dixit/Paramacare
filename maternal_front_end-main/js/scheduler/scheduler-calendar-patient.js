(function ($) {
  ...
  $.fn.patientBookingCalendar = function (opts) {
    ...
    this.hasModified = () => {
      return selection != null && meet != null;
    };

    this.getDoctorInfo = function (practitionerName) {
      var ret = ``;
      ret += `<div class="sch-practitioner-title">
      <span><b>${practitionerName}'s Availability</b></span>
      </div>`;
          ...
          $.each(times.ascending(), function (_, slotTime) {
            ...
            if (!isBefore && isBookedTime(day, date, slotTime)) {
              title =
                ...
                " (" +
                printTime(slotTime) +
                ") is booked.\n * Click other date to reschedule";
    ...
    let render = function () {
      let practitionerName = getQueryParam('practitionerName');
      let ret = instance.getDoctorInfo(practitionerName);
